...
class MemberAdapter(private val listMembers: List<MemberModel>) :RecyclerView.Adapter<MemberAdapter.ViewHolder>() {
    class ViewHolder(val item: View):RecyclerView.ViewHolder(item) {
        val imageUser =item.findViewById<ImageView>(R.id.profile1_pic)
        val imageic =item.findViewById<ImageView>(R.id.callic)
        val name =item.findViewById<TextView>(R.id.profile1_text)
